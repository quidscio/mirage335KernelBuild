...
export ub_setScriptChecksum_contents='3735026340'
...
_check_nv_sequence() {
    ...
    then
        ...
        currentExitStatus=0
        ! sh "$safeTmp"/NVIDIA-Linux-x86_64-"$currentVersion"-custom.run --tmpdir="$safeTmp"/tmp --ui=none --no-questions -j "$currentParallel" --no-cc-version-check -k "$currentLine" -m=kernel && currentExitStatus=1
        ...
        currentExitStatus=0
        ! sh "$safeTmp"/NVIDIA-Linux-x86_64-"$currentVersion".run --tmpdir="$safeTmp"/tmp --ui=none --no-questions -j "$currentParallel" --no-cc-version-check -k "$currentLine" -m=kernel && currentExitStatus=1
        cat /var/log/nvidia-installer.log
        [[ "$currentExitStatus" != "0" ]] && return 1
    fi
